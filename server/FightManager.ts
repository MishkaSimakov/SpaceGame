...
export default class FightManager {
    ...
    protected async chooseProtectors(attacker: Player, target: Player) {
        await new Promise(resolve => {
            this.gameManager.emitToPlayerAndWaitForAnswer(target, 'chooseProtectors', (response: { protector?: [number, number] }) => {
    ...
    protected async askForRunaway(attacker: Player): Promise<boolean> {
        return new Promise(resolve => {
            this.gameManager.emitToPlayerAndWaitForAnswer(attacker, 'willYouRunaway', (response: { tryToRunaway: boolean }) => {
    ...
    protected async chooseWeaponAndTarget(attacker: Player, target: Player) {
        return new Promise(resolve => {
            this.gameManager.emitToPlayerAndWaitForAnswer(attacker, 'chooseWeaponAndTarget', target.link, (response: { weapon: [number, number], target: [number, number] }) => {
